  ...
  <div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer modern-dialog-footer">
        ...
        <el-button type="primary" @click="updateHome" class="btn-submit">保 存</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    findHomeById(id) {
      ...
      this.$http
        .post("/api/productCtl/homeList", { id })
        .then(resp => {
          if (
            resp.data &&
            resp.data.data &&
            ...
            const home = resp.data.data.list[0];
            this.form = {
              id: home.id,
              name: home.name,
              homeTypeId: home.homeType ? home.homeType.id : "",
              price: home.price,
              description: home.description,
              img: home.img
            };
            this.imageUrl = home.img || "";
          } else {
            this.$message({
              message: "获取服务详情失败",
              type: "error",
    ...
    updateHome() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$http
            .post("/api/productCtl/updateHome", this.form)
            .then(resp => {
              if (resp.data.code === 200) {
                this.$message({
                  message: "修改成功",
