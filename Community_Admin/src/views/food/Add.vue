<template>
	<div>
		<el-dialog title="添加餐饮商品" :visible.sync="dialogVisible" width="600px" class="modern-dialog fixed-dialog" :modal="false">
        ...
        <el-button type="primary" @click="addFood()" class="btn-submit">保 存</el-button>
      </span>
		</el-dialog>
	</div>
</template>
	...
	methods:{
		...
		addFood(){
			this.$refs.form.validate((valid) => {
				if (valid) {
					this.$http.post("/api/productCtl/addFood", this.form).then((resp)=>{
						if(resp.data.code == 200){
							this.$message({
								message: '添加成功',
								type: 'success',
