<template>
	<el-dialog 
		...
		:append-to-body="true"
		custom-class="activity-add-dialog"
		@close="closeDialog">
	...
	methods: {
		submitForm() {
			this.$refs.form.validate((valid) => {
				if (valid) {
					this.loading = true;
					this.$http.post('api/communityCtl/addActivity', this.form).then((resp) => {
						this.loading = false;
...
.activity-add-dialog >>> .el-dialog__header {
	flex-shrink: 0;
	padding: 15px 20px !important;
}

.activity-add-dialog >>> .el-dialog__body {
...
.activity-add-dialog >>> .el-dialog__footer {
	flex-shrink: 0;
