  ...
  <view class="purchase-page">
    <view class="hero-banner">
      ...
      <view class="hero-inner">
        <text class="hero-kicker">ERRAND MARKET</text>
        <text class="hero-title">代购服务</text>
      </view>
    </view>
    ...
    <view class="purchase-body">
      <view class="filter-card glass-card">
        <view class="card-header">
          <view>
            <text class="card-title">商品分类</text>
            <text class="card-subtitle">选中分类后，商品列表会即时过滤</text>
          </view>
          <text class="count-pill">{{ filteredProducts.length }}类</text>
        </view>
        ...
        <scroll-view scroll-x class="category-scroll" show-scrollbar="false">
          <view class="category-line">
            ...
            >
              <image class="pill-icon" src="https://news11.oss-cn-chengdu.aliyuncs.com/%E5%85%A8%E9%83%A8%20%281%29.svg" mode="aspectFit"></image>
              <text class="pill-text">全部</text>
            </view>
            <view
              v-for="category in purchaseTypes"
              :key="category.id"
              class="category-pill"
              :class="{ active: currentCategory.id === category.id }"
              @click="selectCategory(category)"
            >
              <image class="pill-icon" :src="category.icon" mode="aspectFit"></image>
      ...
      <view class="selected-bar glass-card">
        ...
        <text class="selected-name">{{ currentCategory.name || '全部商品' }}</text>
      </view>

      <view class="product-section">
        <view class="section-header">
          <view>
            <text class="section-title">代购商品</text>
            <text class="section-subtitle">点击商品看详情，点击加号快速加入购物车</text>
          </view>
        </view>
        ...
        <view v-if="filteredProducts.length" class="product-grid">
          ...
          >
            ...
            <view class="product-content">
              ...
              <text class="product-tag">{{ product.purchasesType && product.purchasesType.name ? product.purchasesType.name : '代购商品' }}</text>
              <text class="product-desc">{{ product.description || getProductDesc(product) }}</text>
            </view>
            <view class="add-fab" @click.stop="addToCart(product)">
        ...
        <view v-else class="empty-card glass-card">
          ...
          <text class="empty-tip">当前分类下还没有代购商品</text>
        </view>
      </view>
    </view>
        ...
        <view class="cart-items">
          <view v-for="item in cartItems" :key="item.id" class="cart-item">
            <view>
              ...
              <text class="cart-item-meta">x{{ item.quantity }}</text>
            </view>
      ...
      <view class="order-modal" @click.stop>
        <view class="modal-top">
          <view>
            ...
            <text class="modal-subtitle">请核对代购商品与配送信息</text>
          </view>
...
export default {
  data() {
    return {
      allCategory: { id: 0, name: '全部商品' },
      currentCategory: { id: 0, name: '全部商品' },
      cartExpanded: false,
      purchaseTypes: [],
      purchases: [],
      cartItems: [],
  ...
  computed: {
    filteredProducts() {
      ...
      return this.purchases.filter(product => product.purchasesType.id === this.currentCategory.id);
    },
  ...
  methods: {
    async getPurchasesTypes() {
      const res = await this.$myRequest({
        url: '/user/ordersCtl/purchasesTypes',
        method: 'GET'
      });
    ...
    async getPurchases() {
      const res = await this.$myRequest({
        url: '/user/ordersCtl/purchasesList',
        method: 'GET'
      });
    ...
    selectProduct(product) {
      uni.showModal({
        title: product.name,
        content: `${this.getProductDesc(product)}\n价格: ¥${Number(product.price).toFixed(2)}/件`,
        confirmText: '加入购物车',
        success: (res) => {
          if (res.confirm) {
    ...
    getProductDesc(product) {
      ...
      return categoryMap[product.purchasesType && product.purchasesType.id] || '优质商品';
    },
    ...
    async submitOrder() {
      try {
        ...
        const orderData = {
          ...
          orderStatus: 0,
          typeid: 2,
          orderPurchasesItems: this.cartItems.map(item => ({
            purchasesId: item.id,
        ...
        const res = await this.$myRequest({
          url: '/user/ordersCtl/createOrderPurchases',
          method: 'POST',
          data: orderData
        });
...
.purchase-page {
  min-height: 100vh;
  background:
    radial-gradient(circle at top left, rgba(255, 205, 84, 0.2), transparent 24%),
    linear-gradient(180deg, #0d1f2d 0%, #12293d 28%, #eef2f6 28%, #eef2f6 100%);
...
.hero-banner {
  ...
  background-image: url('https://bpic.588ku.com/back_pic/05/60/26/485b3f923bc977b.jpg');
...
.hero-mask {
  position: absolute;
  inset: 0;
  background: linear-gradient(135deg, rgba(6, 18, 28, 0.86), rgba(22, 53, 79, 0.45));
}
...
.glass-card {
  ...
  border: 1rpx solid rgba(15, 53, 84, 0.08);
  border-radius: 28rpx;
  box-shadow: 0 16rpx 40rpx rgba(14, 37, 56, 0.1);
}
...
.category-pill {
  ...
  border-radius: 22rpx;
  background: #f8fbff;
  border: 1rpx solid rgba(25, 73, 109, 0.08);
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 10rpx;
...
.category-pill.active {
  background: linear-gradient(135deg, #11304a 0%, #1e5f90 100%);
  box-shadow: 0 12rpx 28rpx rgba(17, 48, 74, 0.22);
}
...
.product-card {
  ...
  border-radius: 28rpx;
  background: #ffffff;
  box-shadow: 0 12rpx 30rpx rgba(18, 49, 75, 0.08);
}
...
.product-tag {
  ...
  padding: 8rpx 14rpx;
  border-radius: 999rpx;
  background: rgba(21, 75, 116, 0.08);
  color: #275170;
  font-size: 20rpx;
}
...
.add-fab {
  ...
  height: 66rpx;
  border-radius: 50%;
  background: linear-gradient(135deg, #ff9d2e 0%, #ff6b2f 100%);
  display: flex;
  align-items: center;
  justify-content: center;
  box-shadow: 0 12rpx 24rpx rgba(255, 120, 37, 0.28);
}
...
.floating-cart {
  ...
  right: 20rpx;
  bottom: 20rpx;
  background: rgba(10, 23, 34, 0.95);
  border-radius: 30rpx;
  box-shadow: 0 22rpx 42rpx rgba(9, 24, 37, 0.3);
  color: #f4fbff;
  z-index: 50;
  overflow: hidden;
}
...
.cart-badge {
  ...
  height: 56rpx;
  border-radius: 50%;
  background: linear-gradient(135deg, #ffb540 0%, #ff7a1e 100%);
  color: #fff;
  display: flex;
  align-items: center;
  justify-content: center;
...
.cart-submit {
  ...
  height: 76rpx;
  border-radius: 999rpx;
  background: linear-gradient(135deg, #ffad33 0%, #ff6f26 100%);
  color: #fff;
  display: flex;
  align-items: center;
  justify-content: center;
...
.modal-item {
  ...
  justify-content: space-between;
  gap: 14rpx;
  border-bottom: 1rpx solid #eef3f7;
}
...
.modal-btn.strong {
  background: linear-gradient(135deg, #11304a 0%, #1f6598 100%);
  color: #fff;
}
</style>
